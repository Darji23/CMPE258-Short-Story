...
class GraphDataset(pl.LightningDataModule):
    def __init__(
        ...
        # We already set self.n_nodes and self.n_edges in load_dataset for PyG
        ...
        self.features, self.unmasked_pred, self.dist = self.prepare_prop_features_logits_and_dist_features(self.edge_index, self.feat, n_hops=cfg.n_hops)
        del self.edge_index
        del self.feat
    ...
    def load_dataset(self, data_init_args):
        ...
        if self.data_source == "ogb":
            ...
            if hasattr(dataset, "train_mask") and hasattr(dataset, "val_mask") and hasattr(dataset, "test_mask"):
                ...
                train_mask, val_mask, test_mask = get_data_split_masks(n_nodes, label, 20 * num_class, seed=self.cfg.seed)
        ...
        from torch_geometric.utils import add_self_loops, remove_self_loops, to_undirected, coalesce
        ...
        # We'll store edge_index instead of g
        self.edge_index = edge_index
        self.n_nodes = n_nodes
        self.n_edges = edge_index.shape[1]

        return None, label, feat, train_mask, val_mask, test_mask, num_class
    ...
    def prepare_prop_features_logits_and_dist_features(self, edge_index, input_feats, n_hops):
        if not os.path.exists(self.cache_f_name):
            edge_index = edge_index.to(self.preprocess_device)
            with timer(f"Computing {self.name} message passing and normalized predictions to file {self.cache_f_name}", logger.info):
                dim = input_feats.size(1)
                num_nodes = self.n_nodes
                LP = torch.zeros(n_hops, num_nodes, dim).to(self.preprocess_device)
                HP = torch.zeros(n_hops, num_nodes, dim).to(self.preprocess_device)
                
                lp_feat = input_feats.to(self.preprocess_device)
                hp_feat = input_feats.to(self.preprocess_device)
                ...
                norm = deg_inv[col]
                adj = torch.sparse_coo_tensor(torch.stack([col, row]), norm, (num_nodes, num_nodes)).to(self.preprocess_device)
                
                for hop_idx in range(n_hops):
                    lp_feat = torch.sparse.mm(adj, lp_feat)
                    hp_out = torch.sparse.mm(adj, hp_feat)
                    hp_feat = hp_feat - hp_out
                    
                    LP[hop_idx] = lp_feat.clone()
                    HP[hop_idx] = hp_feat.clone()
                ...
                unmasked_pred = self.compute_channel_logits(features, self.train_indices, sample=False, device=self.preprocess_device)
        ...
        if not os.path.exists(self.dist_f_name):
            with timer(f"Computing {self.name} conditional gaussian distances and save to {self.dist_f_name}", logger.info):
                y_feat = np.stack([unmasked_pred[c].cpu().numpy() for c in self.cfg.feat_channels], axis=1)
                bsz, n_channel, n_class = y_feat.shape
                dist_feat_dim = n_channel * (n_channel - 1)
                cond_gaussian_prob = np.zeros((bsz, n_channel, n_channel))
                for i in range(bsz):
                    cond_gaussian_prob[i, :, :] = get_entropy_normed_cond_gaussian_prob(y_feat[i, :, :], self.cfg.entropy)
