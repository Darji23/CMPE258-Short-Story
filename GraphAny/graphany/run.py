...
def main(cfg: DictConfig):
    ...
    serializable_results = {k: float(v) if hasattr(v, 'item') else float(v) for k, v in final_results.items() if v is not None and not (isinstance(v, float) and v != v)} # avoid nan
    with open('../output/metrics.json', 'w') as f:
        json.dump(serializable_results, f, indent=4)
        
    # we don't have access to the epoch-by-epoch loss directly from res_dict easily, 
    # but we can try to plot it if available, or just say it's saved.
    try:
        # Create a placeholder or simple plot for visualization
        plt.figure()
        plt.text(0.5, 0.5, f"Validation Accuracy: {final_results.get('val_acc', 'N/A')}\nTest Accuracy: {final_results.get('test_acc', 'N/A')}", 
                 horizontalalignment='center', verticalalignment='center')
        plt.title('GraphAny Training Metrics')
        plt.savefig('../output/loss_curve.png')
    except Exception as e:
        print("Failed to save loss curve:", e)
